...
cantCompile = (code) ->
  throws -> Caffeine.compile code


test "ensure that carriage returns don't break compilation on Windows", ->
  doesNotThrow -> Caffeine.compile 'one\r\ntwo', bare: on

test "--bare", ->
  eq -1, Caffeine.compile('x = y', bare: on).indexOf 'function'
  ok 'passed' is Caffeine.eval '"passed"', bare: on, filename: 'test'

test "header (#1778)", ->
  header = "// Generated by Caffeine #{Caffeine.VERSION}\n"
  eq 0, Caffeine.compile('x = y', header: on).indexOf header

test "header is disabled by default", ->
  header = "// Generated by Caffeine #{Caffeine.VERSION}\n"
  eq -1, Caffeine.compile('x = y').indexOf header
...
test "reference named hasOwnProperty", ->
  Caffeine.compile 'hasOwnProperty = 0; a = 1'
